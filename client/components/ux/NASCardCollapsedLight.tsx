...
interface NASCardCollapsedLightProps {
  title?: string;
  source?: string;
  onExpand?: () => void;
  onClose?: () => void;
...
export default function NASCardCollapsedLight({
  ...
  return (
    <div className="w-full" style={{ maxWidth: nasCard.collapsed.width, margin: '0 auto' }}>
      {/* NAS Solution Card Container - EXACT 904 × 175 */}
      <div 
        style={{
          position: 'relative',
          width: nasCard.collapsed.width,
          height: nasCard.collapsed.height,
          backgroundColor: light.cardBackground, // Off-white #FEFEFF
      ...
      >
        ...
        >
          ...
          >
            <Maximize2 size={cardConfig.controlSize} color={light.controlIcons} strokeWidth={2} />
          </button>
          <button 
            onClick={onClose}
            style={{
            ...
            className="hover:opacity-70"
          >
            <X size={cardConfig.controlSizeLarge} color={light.controlIcons} strokeWidth={2} />
          </button>
        </div>
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            >
              <ChevronDown size={20} color="#7F7F7F" />
            </button>
          </div>
        </div>
      </div>
